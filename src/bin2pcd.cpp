 ...
 * @create date 2018-10-16 15:06:33
 * @modify date 2018-10-16 15:06:33
 * @desc [description]
 */

// usage;
// bin2pcd --folder path/to/bin/files -o path/to/output/dir
...
int main(int argc, char** argv) {
    ...
    desc.add_options()("ascii,a", po::bool_switch(&save_as_ascii),
                       "save pcd as ASCII code. Default : binary. e.g. -a")(
        "file,s", po::value<string>(&single_file),
        "single file. e.g. --file 000001.bin")(
        "folder,d", po::value<string>(&folder),
        "entire folder. e.g. --folder sequences/00/velodyne")(
        "files,m", po::value<vector<string> >(&files)->multitoken(),
        "multiple files. e.g. --files 000001.bin 000002.bin")(
        "outpath,o",
        po::value<string>(&output_path)
            ->default_value(fs::current_path().string()),
        "path to where pcd files are saved. Default is current directory. e.g. "
        "-o output");
    po::variables_map vm;
    ...
    try {
        ...
        if (vm.count("folder")) {
            cerr << folder << endl;
            if (!getFilesList(folder, files_to_be_converted)) return 0;
        }
        if (vm.count("files")) {
        ...
        for (vector<fs::path>::const_iterator iter =
                 files_to_be_converted.begin();
             iter != files_to_be_converted.end(); iter++) {
            CloudXYZI cloud;
            readKittiVelodyne(*iter, cloud);
            saveAsPCD(cloud,
                      o_path / iter->filename().replace_extension("pcd"));
        }
